        ...
        private void CallSetupHelper(string arguments)
        {
            ...
            if (Environment.OSVersion.Version.Major <= 5)
            {
                ...
                if (!osHelper.UserIsAdministrator())
                {
                    ...
                    string[] label = { "Yes", "No" };
                    label[0] = TranslationHelper.Instance.TranslatorInstance.GetTranslation("MessageWindow", "Yes", "Yes");
                    label[1] = TranslationHelper.Instance.TranslatorInstance.GetTranslation("MessageWindow", "No", "No");

                    if (MessageWindow.Show(message, caption, label, MessageWindowButtons.YesNo,
                            MessageWindowIcon.Info) == AdonisUI.Controls.MessageBoxResult.No)
                        return;
                }
            }
            ...
            if (Environment.OSVersion.Version.Major <= 5)
            {
                ...
                if (!osHelper.UserIsAdministrator())
                {
                    ...
                    if (MessageWindow.Show(message, caption, label, MessageWindowButtons.YesNo,
                        ...
                        AdonisUI.Controls.MessageBoxResult.No)
                        return false;
                }
            }
