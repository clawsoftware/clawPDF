    ...
    internal partial class DebugTab
    {
        ...
        private void ShowLogButton_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                string[] label = { "OK" };
                label[0] = TranslationHelper.Instance.TranslatorInstance.GetTranslation("MessageWindow", "Ok", "OK");
        ...
        private bool QuerySaveModifiedSettings()
        {
            ...
            if (response == AdonisUI.Controls.MessageBoxResult.Yes) //Proceed with saved settings
            {
                SaveAppSettings();
                return true;
            }

            if (response == AdonisUI.Controls.MessageBoxResult.No) //Proceed with old settings
                return true;
        ...
        private void LoadIniButton_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageWindow.Show(message, caption, label, MessageWindowButtons.YesNo, MessageWindowIcon.Warning) !=
            ...
            if (!SettingsHelper.CheckValidSettings(settings))
            {
                ...
                string[] labels = { "OK" };
                labels[0] = TranslationHelper.Instance.TranslatorInstance.GetTranslation("MessageWindow", "Ok", "OK");

                MessageWindow.Show(message, caption, labels, MessageWindowButtons.OK, MessageWindowIcon.Error);
                return;
            }
