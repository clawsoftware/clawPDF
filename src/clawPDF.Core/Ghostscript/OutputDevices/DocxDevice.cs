...
{
    /// <summary>
    ///     Extends OutputDevice to create DOCX files
    /// </summary>
    public class DocxDevice : OutputDevice
    {
        private const int DpiMin = 4;
        private const int DpiMax = 2400;

        public DocxDevice(IJob job) : base(job)
        {
        }

        public DocxDevice(IJob job, IFile file, IOsHelper osHelper) : base(job, file, osHelper)
        {
        }

        protected override void AddDeviceSpecificParameters(IList<string> parameters)
        {
            parameters.Add("-sDEVICE=docxwrite");
        ...
        {
            return Job.JobTempFileName + ".docx";
        }
    }
}